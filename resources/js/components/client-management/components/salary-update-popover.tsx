...
const SalaryUpdatePopover: React.FC<Props> = ({
  ...
  const [salary, setSalary] = useState<string>(latestSalary || '');
  const [notes, setNote] = useState<string>(latestNote || '');
  const [saving, setSaving] = useState(false);

  useEffect(() => {
    // Update salary and note on popover open
    if (open) {
  ...
  // For SPA/API, recommend POST to `/api/clients/{acctno}/salary`
  const handleSave = async () => {
    setSaving(true);
    try {
      const res = await fetch(`/api/clients/${acctno}/salary`, {
        method: 'POST',
        headers: {
