...
const PaymentLedgerTable: React.FC<Props> = ({ title = 'Payment Ledger', rows, loading = false, onRefresh, lnnumber, exportMeta }) => {
    ...
    const meta = useMemo(
        () => ({
            ...
            lnnumber: lnnumber || exportMeta?.lnnumber || '',
    ...
    const handleExportCsv = () => {
        ...
        link.href = url;
        link.setAttribute('download', `payments${meta.lnnumber ? `_${meta.lnnumber}` : ''}.csv`);
        link.click();
    ...
    const handleExportExcel = async () => {
        try {
            ...
            const link = document.createElement('a');
            link.href = url;
            link.download = `payments${meta.lnnumber ? `_${meta.lnnumber}` : ''}.xlsx`;
            link.click();
            URL.revokeObjectURL(url);
        } catch (error) {
    ...
    const handleExportPdf = async () => {
        try {
            ...
            doc.save(`payments${meta.lnnumber ? `_${meta.lnnumber}` : ''}.pdf`);
        } catch (err) {
                    ...
                    <Box sx={{ p: 2, pt: 0 }}>
                        ...
                        >
                            <Box
                                sx={{
                                    display: 'grid',
                                    gridTemplateColumns: '1fr 1fr 1fr 0.6fr 1fr',
                                    alignItems: 'center',
                            ...
                            >
                                {['Date', 'Principal', 'Payments', 'CS/CK', 'Balance'].map((text) => (
                                    <Skeleton
                                        key={text}
                                        variant="rounded"
                                        height={14}
                            ...
                            <Box>
                                {Array.from({ length: skeletonRows }).map((_, idx) => (
                                    ...
                                    >
                                        {[55, 60, 60, 40, 60].map((width, colIdx) => (
                                            <Skeleton
                                                key={colIdx}
                                                variant="rounded"
                                                height={14}
