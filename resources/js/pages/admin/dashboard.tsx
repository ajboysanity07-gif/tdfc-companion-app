...
export default function AdminDashboard() {
  ...
  const cardClass = useMemo(
    () => `rounded-2xl border border-gray-200 dark:border-neutral-700 shadow-sm ${theme.card}`,
    [theme.card],
  );
    ...
    <AppLayout breadcrumbs={[{ title: 'Admin Dashboard', href: adminDashboardHref }]}>
      ...
      <div className={`w-full px-4 pb-10 pt-6 sm:px-6 lg:px-8 ${theme.bgClass}`}>
        <div className="flex w-full flex-col gap-6 sm:gap-8">
          {/* Welcome Section */}
          <div className="flex flex-col gap-4 rounded-2xl bg-[#F57979] px-6 py-5 shadow-lg sm:flex-row sm:items-center sm:justify-between">
            <div className="space-y-2">
              <h1 className="text-3xl font-extrabold tracking-tight text-[#FFF172] sm:text-4xl">
                Welcome Back, {user.name.split(' ')[0] ?? 'Admin'} 👋
              </h1>
              <p className="text-white/90">
                You are logged in as <span className="font-semibold uppercase">{user.role}</span>
              </p>
            </div>
            <div className="flex h-16 w-16 items-center justify-center rounded-full bg-white/20 shadow-lg sm:h-20 sm:w-20">
              {user.avatar ? (
          ...
          <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-3">
            <button
              ...
              className={`group flex w-full transform flex-col items-center justify-center p-8 transition-all duration-300 hover:-translate-y-1 hover:shadow-lg sm:p-10 ${cardClass}`}
            >
            ...
            <button
              ...
              className="flex w-full flex-col items-center justify-center rounded-2xl bg-[#F57979] p-8 text-white shadow-md transition-all duration-300 hover:bg-[#f46868] hover:shadow-lg sm:p-10"
            >
          ...
          <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-3">
            {loading ? (
              [1, 2, 3].map((i) => (
                <div key={i} className={`h-24 animate-pulse rounded-2xl shadow-sm ${theme.card}`} />
              ))
            ) : (
              summary && (
                <>
